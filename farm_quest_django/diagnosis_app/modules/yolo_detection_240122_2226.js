...
def detect(save_file_path, plant_name, user_select_plant):    
    ...
    try: 
        ...
        #       'predict_desease_list', tf_predict_desease_list}
        print('tf_predict_desease_list 232323 : ', tf_predict_desease_list)
...
def tf_detect(serialized_results_list, plant_name):
    ...
    disease_keys.append('0')
    print('disease_keys', disease_keys)
    ...
    for idx, p in enumerate(pred_prob):
        ...
        print('예측 : ', disease_values[max_idx])
        print('예측 : ', disease_items[max_idx])
        
        # predict_desease_list = [dict(zip(disease_keys, p.tolist())) for p in pred_prob]
        
        sorted_predict_desease = dict(sorted(zip(disease_values, p.tolist()), key=lambda item: item[1], reverse=True))
        ...
        if Y_t[idx].split('_')[0] == disease_values[max_idx]:
            print('--> True')
        else:
            print('--> False')
    ...
    print(len(predict_desease_list))
    print('predict_desease_list', predict_desease_list)
