...
const UploadResult = () => {
    ...
    const tf_pred_prob = location.state.file_name.tf_pred_prob;
    
    console.log(tf_predict_desease_list)
    console.log(tf_pred_prob)
        ...
        <div>
            ...
            {Object.keys(serialized_results.boxes).map((boxKey, index) => {
                ...
                const url_crops = save_file_name
                    ? `http://127.0.0.1:8000/media/diagnosis/yolo/result_img/${file_name}/crops/${labelName}/${file_name}.jpg`
                    : null;
            ...
            <div>이미지 분류 모델 진단 결과 : {tf_predict_desease_list}</div>
            <div>이미지 분류 모델 진단 확률 : {tf_pred_prob[0]}</div>
        </div>
    );
};
