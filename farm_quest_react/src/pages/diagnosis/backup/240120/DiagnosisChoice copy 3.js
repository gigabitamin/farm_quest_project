...
const DiagnosisChoice = () => {
    ...
    const handlePlantChange = (e) => {
        setSelectedPlantIndex(parseInt(e.target.value, 10));
    };
    ...
    const handleChoiceSubmit = async () => {
        try {
            if (selectedPlantIndex !== null) {
                ...
                const response = await axios.post('http://localhost:8000/save_diagnosis_result/', {
                    user_select_plant: selectedPlantIndex,
                    // 여기에 필요한 다른 데이터도 추가할 수 있습니다.
                });
                console.log(response.data); // 서버로부터의 응답 확인
                alert("선택한 작물: " + plantSpecies[selectedPlantIndex].plant_name);
                history('/upload'); // 업로드 결과 화면으로 이동
            } else {
